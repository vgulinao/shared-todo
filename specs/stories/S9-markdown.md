...
- **AC3** Clicking the rendered description (edit role) opens the editor with the raw Markdown.
...
- **AC5** Raw HTML inside the Markdown is never rendered as HTML (it shows as text), and links open in
  a new tab with `rel="noopener noreferrer"`. This is the XSS boundary of the feature.
...
| AC5        | Component test (jsdom): `<script>` and `<img onerror>` in Markdown render as text; a link has `target=_blank` and `rel=noopener noreferrer` | client |
